...
func TestSkillsShowsCopyReadyPromptsWithoutJSONDump(t *testing.T) {
	...
	for _, want := range []string{"Copy one of these starter prompts", "Make this app Test Store-ready", "Connect my Apple account", "Sync my product catalog to the stores", "create-revenuecat-project skill", "integrate-revenuecat skill", "revenuecat-store-state skill", "revenuecat-status skill"} {
...
func TestSkillsInstallDefaultsToCoreBundleWithoutPicker(t *testing.T) {
	...
	if !slices.Contains(installer.args, "--agent") {
		t.Fatalf("installer args missing --agent: %v", installer.args)
	}
	for _, agent := range defaultToolkitAgents {
		if !slices.Contains(installer.args, agent) {
			t.Errorf("installer args missing supported agent %q: %v", agent, installer.args)
		}
	}
